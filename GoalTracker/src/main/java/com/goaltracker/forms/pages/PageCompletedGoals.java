/*
 * PageCompletedGoals.java
 * - display completed goals page
 *
 * Tyler Moquin
 *
 * VERSION HISTORY
...
public class PageCompletedGoals extends JPanel implements Page {

    public PageCompletedGoals() {
        initComponents();
        ...
        if (goals != null) {
            ...
            if (count > 0) {
                for (int i = count - 1; i >= 0; i--) {
                    Goal g = goals.get(i);
                    if (g.getIsSubGoal() == false && g.getIsCompleted()) {
                        if (g instanceof AccumulationGoal) {
                            summaryPanel.add(new CardAccumulationGoal(g, false, true, false, false));
                        } else if (g instanceof NumericGoal) {
                            summaryPanel.add(new CardNumericGoal(g, false, true, false, false));
                        } else if (g instanceof BinaryGoal) {
                            summaryPanel.add(new CardBinaryGoal(g, false, true, false, false));
                        } else if (g instanceof TaskListGoal) {
                            summaryPanel.add(new CardTaskListGoal(g, false, true, false, false));
                        }
                        numDisplayed++;
                    }
                }
    ...
    private void initComponents() {
        ...
        newGoalLabel.setText("Completed");
        ...
        titleLayout.setHorizontalGroup(
            ...
            .addGroup(titleLayout.createSequentialGroup()
                .addContainerGap()
                .addComponent(newGoalLabel, javax.swing.GroupLayout.DEFAULT_SIZE, 721, Short.MAX_VALUE)
                .addGap(11, 11, 11))
        );
