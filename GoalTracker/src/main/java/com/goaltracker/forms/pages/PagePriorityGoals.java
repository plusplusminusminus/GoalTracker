/*
 * PagePriorityGoals.java
 * - display priority goals page
 *
 * Tyler Moquin
 *
 * VERSION HISTORY
...
public class PagePriorityGoals extends JPanel implements Page {
        ...
        if (goals != null) {
            ...
            if (count > 0) {
                for (int i = count - 1; i >= 0; i--) {
                    Goal g = goals.get(i);
                    if (!(g.getIsSubGoal() == true) && !(g.getIsAbandoned()) && !(g.getIsCompleted()) && (g.getPriority() == PriorityLevel.HIGH)) {
